...
from senaite.instruments.instruments.xrf.xrf.xrf import importer
...
IFACE = (
    ...
    ".xrf.xrf.xrf.importer"
)
TITLE = "KRF"

here = abspath(dirname(__file__))
path = join(here, "files", "instruments", "xrf")
fn1 = join(path, "Test2.txt")
...
class TestKRF(DataTestCase):
    def setUp(self):
        ...
        self.instrument = self.add_instrument(
            title=TITLE,
            InstrumentType=self.add_instrumenttype(title="KRF"),
            Manufacturer=self.add_manufacturer(title="KRF"),
